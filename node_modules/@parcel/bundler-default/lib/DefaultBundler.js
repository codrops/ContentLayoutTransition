    ...
    enter(node, context, actions) {
      if (node.type === 'asset') {
        ...
        for (let childAsset of assets) {
          ...
          if (dependency.priority === 'lazy' || childAsset.bundleBehavior === 'isolated' // An isolated Dependency, or Bundle must contain all assets it needs to load.
          ...
          } else if (parentAsset.type !== childAsset.type || dependency.priority === 'parallel' || childAsset.bundleBehavior === 'inline') {
            ...
            if (parentAsset.type !== childAsset.type && entries.has(bundleGroupRootAsset) && canMerge(bundleGroupRootAsset, childAsset) && dependency.bundleBehavior == null && manualSharedBundleKey == null //exclude MSBs for merging
            ) {
...
async function loadBundlerConfig(config, options) {
  ...
  if (modeConfig.unstable_manualSharedBundles) {
    ...
    (0, _assert().default)(nameSet.size == nameArray.length, 'The name field must be unique for property unstable_manualSharedBundles');
  }
